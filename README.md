...
| **The assistant looking at a photograph** | off, *and* off per family | Two locks. Configure `[ai.vision]` AND the family's owner turns `ai_vision` on — off by default for every family, including ones that already exist. Even then only a photo the member attached, in their own assistant chat, to the question being answered: at most four, at most 5 MiB each, never from the family chat and never one anybody else sent. An `@ai` mention never sends a picture at all. |
| **The assistant making a picture** | off | Configure `[ai.images]`; a member then writes `/draw something`. What leaves is the words after `/draw` and nothing else — not the conversation, not the system prompt. |
