...
extension VoIPPushRegistrar: PKPushRegistryDelegate {
    ...
    private func handleIncoming(_ push: IncomingCallPush?, completion: @escaping () -> Void) {
        ...
        guard let push else {
            ...
            let uuid = UUID()
            callKit.reportIncoming(callID: uuid, peerUserID: nil, peerName: String(localized: "Unknown caller"), hasVideo: false) { _ in
                completion()
            }
        ...
        let name = peer.map { $0.name == String(localized: "Someone") && !push.callerName.isEmpty ? push.callerName : $0.name }
            ?? push.callerName
        callKit.reportIncoming(callID: uuid, peerUserID: push.fromUserID, peerName: name, hasVideo: push.video) { _ in
            completion()
        }
